...
class AuthController extends Controller
{
    public function registro(Request $request)
    {
        try {
            ...
            $verificacionURL = URL::temporarySignedRoute(
                'verification.verify',
                now()->addMinutes(60),
                ['id' => $usuario->id, 'hash' => sha1($usuario->correo)]
            );

            Mail::to($usuario->correo)->send(new VerifyEmail($usuario, $verificacionURL));
                ...
                if ($usuario->email_verified_at != null) {
                    ...
                    if ($usuario->validado) {
                        ...
                        return response()->json([
                            ...
                            'mensaje' => 'Tu cuenta aún no ha sido validada por el Administrador '
                        ], Response::HTTP_FORBIDDEN);
                    }
                } else {
                    return response()->json([

                        'mensaje' => ' Debes de verificar la cuenta desde correo electronico ',
